...
const defaultUserName = function (email) {
    var username = email.split('@')[0];
    return 'g/' + username;
}


//if something didnt work check the server port on the request
function userRegister(){
	...
	const submitAccount = (e) => {
		e.preventDefault()
		// fetch('http://localhost:5862/u/register',{
		// 	method: "POST",
		// 	credentials: "include",
		// 	headers: {
	...
	return (
		...
		<form onSubmit={submitAccount}>
			<img className="h-20 w-20 mx-auto mt-10" src={logo}></img>
			<h1 className="text-4xl font-bold text-center mx-5 my-6">Sign Up for a new account</h1>
			<div className="space-y-5 sm:mx-auto mx-7 flex flex-col items-center"> 
			  <div className="">
			    <label htmlFor="emailInput" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Email</label>
			    <div className="flex">
			      ...
			      <input onChange={emailState} type="email" id="emailInput" className="rounded-none rounded-r-lg bg-gray-50 shadow-sm text-gray-900 focus:ring-blue-500 focus:border-blue-500 px-7 text-sm border-gray-300 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Email"></input>
			    </div>
			  </div>
			  ...
			  <div>
			    ...
			    <div className="flex">
			      ...
			      <input onChange={passwordState} type="password" id="website-admin" className="rounded-none rounded-r-lg bg-gray-50 shadow-sm text-gray-900 focus:ring-blue-500 focus:border-blue-500 px-7 block min-w-0 text-sm border-gray-300 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Password"></input>
			    </div>
			  </div>
			  <button type="submit" className="bg-[#6902FF] sm:w-64 w-full py-2 font-semibold text-white rounded">Sign up</button>
			  {/* OAUTH */}
			  ...
			  <span>Already have an Account?, <a href="/user/login" className="font-bold">Sign in</a> </span>
